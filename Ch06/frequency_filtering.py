...
def frequency_filtering( f, filter, D0, order ):
	...
	if filter == 1:							# 理想低通濾波器：截止頻率外的頻率歸零
		for u in range( nr ):
			for v in range( nc ):
	...
	elif filter == 2:						# 理想高通濾波器：截止頻率內的頻率歸零
		for u in range( nr ):
			for v in range( nc ):
				dist = np.sqrt( ( u - nr / 2 ) * ( u - nr / 2 ) +
				                ( v - nc / 2 ) * ( v - nc / 2 ) )
	...
	elif filter == 3:						# 高斯低通濾波器：平滑的低通頻率衰減
		for u in range( nr ):
			for v in range( nc ):
				dist = np.sqrt( ( u - nr / 2 ) * ( u - nr / 2 ) +
				                ( v - nc / 2 ) * ( v - nc / 2 ) )
	...
	elif filter == 5:						# 巴特沃斯低通濾波器：可調陡降特性
		for u in range( nr ):
			for v in range( nc ):
				dist = np.sqrt( ( u - nr / 2 ) * ( u - nr / 2 ) +
				                ( v - nc / 2 ) * ( v - nc / 2 ) )
	...
	elif filter == 6:						# 巴特沃斯高通濾波器：1 - 巴特沃斯低通
		for u in range( nr ):
			for v in range( nc ):
				dist = np.sqrt( ( u - nr / 2 ) * ( u - nr / 2 ) +
				                ( v - nc / 2 ) * ( v - nc / 2 ) )
