...
def band_filtering( f, filter, D0, width, order ):
	...
	if filter == 1:							# 理想帶阻濾波器：去除中心頻率 D0 附近 ±width/2 的頻段
		for u in range( nr ):
			for v in range( nc ):
	...
	elif filter == 2:						# 理想帶通濾波器：僅保留中心頻率 D0 附近的頻段
		for u in range( nr ):
			for v in range( nc ):
				dist = np.sqrt( ( u - nr / 2 ) * ( u - nr / 2 ) +
				                ( v - nc / 2 ) * ( v - nc / 2 ) )
	...
	elif filter == 5:						# 巴特沃斯帶阻濾波器：可調陡降特性
		for u in range( nr ):
			for v in range( nc ):
				dist = np.sqrt( ( u - nr / 2 ) * ( u - nr / 2 ) +
				                ( v - nc / 2 ) * ( v - nc / 2 ) )
...
def main( ):
	...
	# 使用帶阻濾波器（濾波器1）去除中心頻率100、寬度20的雜訊頻段
