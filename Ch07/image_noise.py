...
def uniform_noise( f, scale ):		# 均勻雜訊：每個像素加上 [0, scale] 範圍內的均勻隨機值
	g = f.copy( )
	nr, nc = f.shape[:2]
	for x in range( nr ):
		for y in range( nc ):
...
def exponential_noise( f, scale ):	# 指數雜訊：加入參數為 scale 的指數分布隨機值
	g = f.copy( )
	nr, nc = f.shape[:2]
	for x in range( nr ):
		for y in range( nc ):
...
def rayleigh_noise( f, scale ):		# 瑞雷雜訊：加入瑞雷分布的隨機值
	g = f.copy( )
	nr, nc = f.shape[:2]
	for x in range( nr ):
		for y in range( nc ):
...
def salt_pepper_noise( f, probability ):  # 鹽與胡椒雜訊：以機率隨機將像素設為 0 或 255
	g = f.copy( )
	nr, nc = f.shape[:2]
	for x in range( nr ):
		for y in range( nc ):
			value = uniform( 0, 1 )
			# 前 probability/2 的機率設為黑色（胡椒）
			if value > 0 and value <= probability / 2:
				g[x,y] = 0
			# 後 probability/2 的機率設為白色（鹽）
